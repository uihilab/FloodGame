...
import { createMeshes } from "./loadModels.js?v=20";
...
async function main(opts, list_of_files, game_graphics_opt) {
    ...
    function init() {
        ...
        // Add a solid dark grass green underplate plane to hide skybox showing through any micro-gaps
        var underplateGeom = new THREE.PlaneGeometry(6000, 6000);
        var underplateMat = new THREE.MeshStandardMaterial({
            color: new THREE.Color("#16321b"), // rich dark green soil color
            roughness: 0.95,
            metalness: 0.05
        });
        var underplate = new THREE.Mesh(underplateGeom, underplateMat);
        underplate.rotation.x = -Math.PI / 2;
        underplate.position.y = -2; // slightly below water/terrain minimum height
        underplate.receiveShadow = true;
        scene.add(underplate);
            ...
            for (var column = 0; column < numberOfColumns; column++){
                ...
                if (obj != 0){
                    ...
                    if (obj.type == "tree"){
                        ...
                        transform.position.set(
                            x,
                            groundTiles[row][column].elevation + 5.0,
                            z - 25);
                        transform.updateMatrix();
                        ...
                        transform.position.set(
                            x,
                            groundTiles[row][column].elevation + 5.5,
                            z);
                        transform.updateMatrix();
