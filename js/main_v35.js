...
import { createMeshes } from "./loadModels.js?v=34";
...
async function main(opts, list_of_files, game_graphics_opt) {
    ...
    function createWorld() {
        ...
        var groundColors = {
            ...
            "building": new THREE.Color("#41472B"),   // Reverted to original olive green color!
        ...
        for (var r = 0; r < numberOfRows; r++) {
            for (var c = 0; c < numberOfColumns; c++) {
                ...
                if (obj.type == "water") {
                    ...
                    waterVertexIndex += 4;
                    // Exclude terrain quads under roads and parking lots to prevent Z-fighting and clipping!
                    if (obj.type === "road" || obj.type === "parking_lot") {
                        // Skip rendering this terrain quad entirely
                    } else {
                        y0 = getCornerElevation(r, c);
                        y1 = getCornerElevation(r + 1, c);
                        y2 = getCornerElevation(r, c + 1);
                        ...
                        // Offset building pad quads down slightly to prevent clipping and keep houses flush
        ...
        // Add vertical side walls to make the map look like a solid 3D block and close the boundary gaps
        for (var r = 0; r < numberOfRows; r++) {
            for (var c = 0; c < numberOfColumns; c++) {
                var obj = groundTiles[r][c];
                var [cx, cz] = calculatePosition(r, c);
                ...
                var y0 = getCornerElevation(r, c);
                var y1 = getCornerElevation(r + 1, c);
                var y2 = getCornerElevation(r, c + 1);
                var y3 = getCornerElevation(r + 1, c + 1);

                var color = groundColors[obj.type] || groundColors["parks"];
                // Darken the side walls slightly for a nice 3D lighting effect
                var sideColor = color.clone().multiplyScalar(0.7);

                // 1. Top border (row == 0): edge is (x0, y0, z0) -> (x1, y1, z0)
                if (r === 0) {
                    verticesLand.push(x0, y0, z0);
                    verticesLand.push(x0, underY, z0);
                    verticesLand.push(x1, y1, z0);
                    verticesLand.push(x1, underY, z0);

                    for (var i = 0; i < 4; i++) {
                        colorsLand.push(sideColor.r, sideColor.g, sideColor.b);
                    }

                    indicesLand.push(landVertexIndex + 0, landVertexIndex + 1, landVertexIndex + 2);
                    indicesLand.push(landVertexIndex + 2, landVertexIndex + 1, landVertexIndex + 3);
                    landVertexIndex += 4;
                }

                // 2. Bottom border (row == numberOfRows - 1): edge is (x2, y2, z2) -> (x3, y3, z2)
                if (r === numberOfRows - 1) {
                    verticesLand.push(x2, y2, z2);
                    verticesLand.push(x2, underY, z2);
                    verticesLand.push(x3, y3, z2);
                    verticesLand.push(x3, underY, z2);

                    for (var i = 0; i < 4; i++) {
                        colorsLand.push(sideColor.r, sideColor.g, sideColor.b);
                    }

                    // Reverse triangle order to face outwards
                    indicesLand.push(landVertexIndex + 0, landVertexIndex + 2, landVertexIndex + 1);
                    indicesLand.push(landVertexIndex + 1, landVertexIndex + 2, landVertexIndex + 3);
                    landVertexIndex += 4;
                }

                // 3. Left border (column == 0): edge is (x0, y0, z0) -> (x2, y2, z2)
                if (c === 0) {
                    verticesLand.push(x0, y0, z0);
                    verticesLand.push(x0, underY, z0);
                    verticesLand.push(x2, y2, z2);
                ...
                // 4. Right border (column == numberOfColumns - 1): edge is (x1, y1, z0) -> (x3, y3, z2)
            ...
            for (var column = 0; column < numberOfColumns; column++){
                ...
                if (obj != 0){
                    ...
                    if (obj.type == "tree"){
                        ...
                        var roadY1 = y0 * (1 - u1) * (1 - v1) + y1 * u1 * (1 - v1) + y2 * (1 - u1) * v1 + y3 * u1 * v1 + 0.25;
                        ...
                        var roadY2 = y0 * (1 - u2) * (1 - v2) + y1 * u2 * (1 - v2) + y2 * (1 - u2) * v2 + y3 * u2 * v2 + 0.25;
