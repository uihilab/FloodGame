...
`;
    ...
    // Initialize the model when the user opens the AI Tutor drawer
    const tutorTrigger = document.querySelector('.tutor-trigger-btn');
    if (tutorTrigger) {
        tutorTrigger.addEventListener("click", () => {
            if (!worker) {
                initModel();
            }
        });
